...
function createPoints(nu) {
  ...
  for (var i = 0; i < nu; i++) {if (window.CP.shouldStopExecution(2)){break;}
    ...
    for (var j = 0; j < 2 * nv; j++) {if (window.CP.shouldStopExecution(1)){break;}
...
function createMatcap(n) {
  ...
  var hue = Math.random();
  for (var i = 0; i < n * n; i++) {if (window.CP.shouldStopExecution(3)){break;}
...
!function (e) {
  ...
  return function e(t, n, r) {
    ...
    }for (var o = "function" == typeof require && require, a = 0; a < r.length; a++) {if (window.CP.shouldStopExecution(5)){break;}
      i(r[a]);
    }
        ...
        function n() {
          ...
          Object.keys(e).forEach(function (t) {
            p[t] = e[t];
              ...
              r = p.up,
              i = p.dtheta,
              o = p.dphi;p.theta += i, p.phi = f(p.phi + o, -Math.PI / 2, Math.PI / 2), p.distance = f(p.distance + L, b, E), p.dtheta = c(i), p.dphi = c(o), L = c(L);for (var a = p.theta, s = p.phi, d = Math.exp(p.distance), v = d * Math.sin(a) * Math.cos(s), l = d * Math.cos(a) * Math.cos(s), m = d * Math.sin(s), h = 0; h < 3; ++h) {if (window.CP.shouldStopExecution(6)){break;}
